...
Airgap installations where Rancher is configured to use the packaged copy of Helm system charts ([`useBundledSystemChart=true`](../../installation-and-upgrade/other-installation-methods/air-gapped/install-rancher-ha.md#helm-chart-options-for-air-gap-installations)) will only refer to the [system-chart](https://github.com/rancher/system-charts) repository that comes bundled and will not be able to be refreshed or synced.
...
If you want to upgrade an installed legacy app, the [legacy feature flag](../../rancher-admin/experimental-features/experimental-features.md) must be turned on. This flag is automatically turned on if you had a legacy app already running before you upgraded Rancher.

1. Enable the [legacy feature flag](../../rancher-admin/experimental-features/experimental-features.md), if it isn't enabled already.
1. Click **☰ > Cluster Management**.

...
:::caution
Pod Security Policies are only available in Kubernetes until v1.24. [Pod Security Standards](../psa-pss.md) are the built-in alternative.
:::
...
Rancher can only assign PSPs for clusters that are [launched using RKE](../../cluster-deployment/launch-kubernetes-with-rancher.md).

...
For more best practices for downstream clusters, refer to the [production checklist](../../cluster-deployment/production-checklist/production-checklist.md) or our [best practices guide.](../../installation-and-upgrade/best-practices/best-practices.md)

## Architecture for an Authorized Cluster Endpoint (ACE)

If you are using an [authorized cluster endpoint (ACE),](communicating-with-downstream-clusters.md#4-authorized-cluster-endpoint) we recommend creating an FQDN pointing to a load balancer which balances traffic across your nodes with the `controlplane` role.

If you are using private CA signed certificates on the load balancer, you have to supply the CA certificate, which will be included in the generated kubeconfig file to validate the certificate chain. See the documentation on [kubeconfig files](../../cluster-admin/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.md) and [API keys](../../rancher-admin/users/settings/api-keys.md#creating-an-api-key) for more information.

ACE support is available for registered RKE2 and K3s clusters. To view the manual steps to perform on the downstream cluster to enable the ACE, click [here](../../cluster-deployment/register-existing-clusters.md#authorized-cluster-endpoint-support-for-rke2-and-k3s-clusters).

...
有关使用 Docker 安装的 Rancher 的升级说明，请参见[本页。](../getting-started/installation-and-upgrade/other-installation-methods/rancher-on-a-single-node-with-docker/upgrade-docker-installed-rancher.md)
...
不支持 _升级_ 或 _升级到_ [rancher-alpha 仓库](../getting-started/installation-and-upgrade/resources/choose-a-rancher-version.md#helm-chart-仓库)中的任何 Chart。
### Helm 版本
...
[仅适用于离线安装](other-installation-methods/air-gapped/air-gapped.md)：为新的 Rancher Server 版本收集和推送镜像。使用你需要针对 Rancher 版本升级的镜像，按照步骤[推送镜像到私有镜像仓库](other-installation-methods/air-gapped/publish-images.md)。
...
使用[备份应用](../rancher-admin/back-up-restore-and-disaster-recovery/back-up.md)来备份 Rancher。
...
1. 获取你用来安装 Rancher 的仓库名称。
   ...
   关于仓库及其区别，请参见 [Helm Chart 仓库](../getting-started/installation-and-upgrade/resources/choose-a-rancher-version.md#helm-chart-仓库)。
   ...
   如果你想切换到不同的 Helm Chart 仓库，请按照[切换仓库步骤](../getting-started/installation-and-upgrade/resources/choose-a-rancher-version.md#切换到不同-helm-chart-仓库)进行操作。如果你要切换仓库，请先再次列出仓库，再继续执行步骤 3，以确保添加了正确的仓库。
...
如果你在离线环境中安装 Rancher，请跳过本页的其余部分，按照[本页](other-installation-methods/air-gapped/upgrades.md)上的说明渲染 Helm 模板。

...
按照以下步骤轮换[安装在 Kubernetes 集群上](../install-rancher.md)、由 Rancher 使用的 SSL 证书和私有 CA，或转用由私有 CA 签发的 SSL 证书。
...
```
:::note 重要：
由于证书由私有 CA 签发，因此确保在 `values.yaml` 文件中设置了 [`privateCA: true`](../references/helm-chart-options.md#常用选项) 是非常重要的。
:::
...
- 方法 2：适用于集群与 Rancher 失去连接，但所有集群都启用了 [Authorized Cluster Endpoint](../../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md) (ACE) 的情况。
...
为每个下游集群使用 Kubeconfig 更新两个 Agent 部署的环境变量。如果集群启用了 [ACE](../../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md)，你可以[调整 kubectl 上下文](../../cluster-admin/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.md#直接使用下游集群进行身份验证)，从而直接连接到下游集群。
...
2. 使用以下其中一种方法，使用 kubeconfig 为下游集群运行上一步中的 kubectl 命令：
   1. 如果集群启用了 [ACE](../../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md)，你可以[调整上下文](../../cluster-admin/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.md#直接使用下游集群进行身份验证)，从而直接连接到下游集群。
...
在 Rancher UI 的[持续交付](../../integrations/fleet/overview.md#在-rancher-ui-中访问-fleet)中，为集群选择“强制更新”，来允许下游集群中的 fleet-agent 成功连接到 Rancher。
...
1. 更新 `values.yaml` 文件中的当前 Helm 值：
   ...
   1. 根据需要调整 `ingress.tls.source` 字段。有关更多信息，请参阅 [Chart 选项](../references/helm-chart-options.md#常用选项)。以下是一些示例：

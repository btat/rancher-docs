...
Follow these steps to rotate an SSL certificate and private CA used by Rancher [installed on a Kubernetes cluster](../install-rancher.md), or migrate to an SSL certificate signed by a private CA.
...
The below steps update the Helm values for the Rancher chart, so the Rancher pods and ingress are reconfigured to use the new private CA certificate created in Step 1 & 2.
   ...
   ```
   :::note Important:
   As the certificate is signed by a private CA, it is important to ensure [`privateCA: true`](../references/helm-chart-options.md#common-options) is set in the `values.yaml` file.
   :::
...
If the clusters have lost connection to Rancher but [Authorized Cluster Endpoint](../../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md) (ACE) is enabled on all clusters, then go with method 2.
...
Using a Kubeconfig for each downstream cluster update the environment variable for the two agent deployments. If the [ACE](../../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md) is enabled for the cluster, [the kubectl context can be adjusted](../../cluster-admin/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.md#authenticating-directly-with-a-downstream-cluster) to connect directly to the downstream cluster.
...
Repeat the below steps for each downstream cluster:
  ...
  2. Run the kubectl command from the previous step using a kubeconfig for the downstream cluster with one of the following methods:
      1. If the [ACE](../../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md) is enabled for the cluster, [the context can be adjusted](../../cluster-admin/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.md#authenticating-directly-with-a-downstream-cluster) to connect directly to the downstream cluster
...
Select 'Force Update' for the clusters within the [Continuous Delivery](../../integrations/fleet/overview.md#accessing-fleet-in-the-rancher-ui) view of the Rancher UI to allow the fleet-agent in downstream clusters to successfully connect to Rancher.
...
The below steps update the Helm values for the Rancher chart, so the Rancher pods and ingress are reconfigured to use the new certificate created in Step 1.
  ...
  1. Update the current Helm values in the `values.yaml` file:
      ...
      1. Adjust the `ingress.tls.source` field as necessary. Please [refer to the chart options](../references/helm-chart-options.md#common-options) for more details. Here are some examples:

...
- **RKE:** For the tutorial to install an RKE Kubernetes cluster, refer to [this page.](install-kubernetes/rke1-for-rancher.md) For help setting up the infrastructure for a high-availability RKE cluster, refer to [this page.](infrastructure-setup/ha-rke1-kubernetes-cluster.md)
- **K3s:** For the tutorial to install a K3s Kubernetes cluster, refer to [this page.](install-kubernetes/k3s-for-rancher.md) For help setting up the infrastructure for a high-availability K3s cluster, refer to [this page.](infrastructure-setup/ha-k3s-kubernetes-cluster.md)
- **RKE2:** For the tutorial to install an RKE2 Kubernetes cluster, refer to [this page.](install-kubernetes/rke2-for-rancher.md) For help setting up the infrastructure for a high-availability RKE2 cluster, refer to [this page.](infrastructure-setupp/ha-rke2-kubernetes-cluster.md)
- **Amazon EKS:** For details on how to install Rancher on Amazon EKS, including how to install an Ingress controller so that the Rancher server can be accessed, refer to [this page.](hosted-kubernetes/rancher-on-amazon-eks.md)
- **AKS:** For details on how to install Rancher with Azure Kubernetes Service, including how to install an Ingress controller so that the Rancher server can be accessed, refer to [this page.](hosted-kubernetes/rancher-on-aks.md)
- **GKE:** For details on how to install Rancher with Google Kubernetes Engine, including how to install an Ingress controller so that the Rancher server can be accessed, refer to [this page.](hosted-kubernetes/rancher-on-gke.md) GKE has two modes of operation when creating a Kubernetes cluster, Autopilot and Standard mode. The cluster configuration for Autopilot mode has restrictions on editing the kube-system namespace. However, Rancher needs to create resources in the kube-system namespace during installation. As a result, you will not be able to install Rancher on a GKE cluster created in Autopilot mode.
...
- [helm](https://docs.helm.sh/using_helm/#installing-helm) - Package management for Kubernetes. Refer to the [Helm version requirements](requirements/helm-version-requirements.md) to choose a version of Helm to install Rancher. Refer to the [instructions provided by the Helm project](https://helm.sh/docs/intro/install/) for your specific platform.
...
For systems without direct internet access, see [Air Gap: Kubernetes install](other-installation-methods/air-gapped/install-rancher-ha.md).
...
To choose a version of Helm to install Rancher with, refer to the [Helm version requirements](requirements/helm-version-requirements.md)
...
> You should skip this step if you are bringing your own certificate files (option `ingress.tls.source=secret`), or if you use [TLS termination on an external load balancer](references/helm-chart-options.md#external-tls-termination).
...
Now that Rancher is deployed, see [Adding TLS Secrets](resources/tls-secrets.md) to publish the certificate files so Rancher and the Ingress controller can use them.
...
- [HTTP Proxy](references/helm-chart-options.md#http-proxy)
- [Private Container Image Registry](references/helm-chart-options.md#private-registry-and-air-gap-installs)
- [TLS Termination on an External Load Balancer](references/helm-chart-options.md#external-tls-termination)

See the [Chart Options](references/helm-chart-options.md) for the full list of options.
...
Doesn't work? Take a look at the [Troubleshooting](troubleshooting/troubleshooting.md) Page

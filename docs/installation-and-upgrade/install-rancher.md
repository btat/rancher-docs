...
- **RKE**：[安装 RKE Kubernetes 集群的教程](install-kubernetes/rke1-for-rancher.md)；[为高可用 RKE 集群设置基础设施的教程](infrastructure-setup/ha-rke1-kubernetes-cluster.md)。
- **K3s**：[安装 K3s Kubernetes 集群的教程](install-kubernetes/k3s-for-rancher.md)；[设置高可用 K3s 集群的基础设施的教程](infrastructure-setup/ha-k3s-kubernetes-cluster.md)。
- **RKE2:** ：[安装 RKE2 Kubernetes 集群的教程](install-kubernetes/rke2-for-rancher.md)；[设置高可用 RKE2 集群的基础设施的教程](infrastructure-setup/ha-rke2-kubernetes-cluster.md)。
- **Amazon EKS**：[在 Amazon EKS 上安装 Rancher 以及如何安装 Ingress Controller 以访问 Rancher Server](hosted-kubernetes/rancher-on-amazon-eks.md)。
- **AKS**：[使用 Azure Kubernetes 服务安装 Rancher 以及如何安装 Ingress Controller 以访问 Rancher Server](hosted-kubernetes/rancher-on-aks.md)。
- **GKE**：有关如何使用 GKE 安装 Rancher，包括如何安装 Ingress Controller 以便可以访问 Rancher Server，请参阅[此页面](hosted-kubernetes/rancher-on-gke.md)。GKE 创建 Kubernetes 集群时有两种运行模式，分别是 Autopilot 和 Standard 模式。Autopilot 模式的集群配置对编辑 kube-system 命名空间有限制。但是，Rancher 在安装时需要在 kube-system 命名空间中创建资源。因此，你将无法在以 Autopilot 模式创建的 GKE 集群上安装 Rancher。
...
- [Helm](https://docs.helm.sh/using_helm/#installing-helm) - Kubernetes 的包管理器。请参见 [Helm 版本要求](requirements/helm-version-requirements.md)选择 Helm 版本来安装 Rancher。请为你的具体平台参见 [Helm 项目提供的说明](https://helm.sh/docs/intro/install/)。
...
如果系统无法直接访问互联网，请参见[离线环境：Kubernetes 安装](other-installation-methods/air-gapped/install-rancher-ha.md)。

如果要指定安装的 Rancher 版本，请参见[选择 Rancher 版本](../getting-started/installation-and-upgrade/resources/choose-a-rancher-version.md)。

如果要指定用于安装 Rancher 的 Helm 版本，请参见[Helm 版本要求](requirements/helm-version-requirements.md)。
...
如果你想在外部终止 SSL/TLS，请参见[外部负载均衡器的 TLS 终止](references/helm-chart-options.md#外部-tls-终止)。
...
> 如果你使用自己的证书文件（`ingress.tls.source=secret`）或使用[外部负载均衡器的 TLS 终止](references/helm-chart-options.md#外部-tls-终止)，你可以跳过此步骤。
...
**添加 TLS 密文（千万不要遗漏此步骤）**：现在 Rancher 已经完成部署，你还需要参考[添加 TLS 密文](resources/tls-secrets.md)发布证书文件，以便 Rancher 和 Ingress Controller 可以使用它们。
...
- [HTTP 代理](references/helm-chart-options.md#http-代理)
- [私有容器镜像仓库](references/helm-chart-options.md#私有仓库和离线安装)
- [外部负载均衡器上的 TLS 终止](references/helm-chart-options.md#外部-tls-终止)

如需获取完整的选项列表，请参见 [Chart 选项](references/helm-chart-options.md)。
...
如果遇到任何问题，请参见[故障排除](troubleshooting/troubleshooting.md)。

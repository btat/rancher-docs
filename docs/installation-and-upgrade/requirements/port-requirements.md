...
不同的下游集群的启动方式有不同的端口要求。下面的每个标签都列出了不同[集群类型](../../cluster-deployment/cluster-deployment.md)所需打开的端口。

下图描述了为每个[集群类型](../../cluster-deployment/cluster-deployment.md)打开的端口。
...
有关 Harvester 端口要求的更多信息，请参阅[此处](../../integrations/harvester/overview.md#端口要求)。
...
下表描述了节点在[云提供商](../../cluster-deployment/infra-providers/infra-providers.md)中创建的情况下，[Rancher 启动 Kubernetes](../../cluster-deployment/launch-kubernetes-with-rancher.md) 的端口要求。
...
下表描述了使用[自定义节点](../../cluster-deployment/custom-clusters/custom-clusters.md)的情况下，[Rancher 启动 Kubernetes](../../cluster-deployment/launch-kubernetes-with-rancher.md) 的端口要求。
...
下表描述了[托管集群](../../cluster-deployment/hosted-kubernetes/hosted-kubernetes.md)的端口要求。
...
下表描述了[注册集群](../../cluster-deployment/register-existing-clusters.md)的端口要求。
...
当你使用 [AWS EC2 Node Driver](../../cluster-deployment/infra-providers/aws/aws.md) 在 Rancher 中配置集群节点时，你可以让 Rancher 创建一个名为 `rancher-nodes` 的安全组。以下规则会自动添加到该安全组中。

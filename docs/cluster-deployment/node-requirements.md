...
如果 Rancher 安装在高可用的 Kubernetes 集群上，Rancher Server 的三节点集群和下游集群有不同的要求。有关 Rancher 的安装要求，请参考[安装文档](../installation-and-upgrade/requirements/requirements.md)中的节点要求。
...
如果你想使用 ARM64，请参阅[在 ARM64 上运行（实验功能）。](../rancher-admin/experimental-features/rancher-on-arm64.md)
...
SUSE Linux 可能有一个防火墙，默认情况下会阻止所有端口。在这种情况下，请按照[步骤](../installation-and-upgrade/requirements/port-requirements.md#打开-suse-linux-端口)打开将主机添加到自定义集群所需的端口。

### Flatcar Container Linux 节点

使用 Flatcar Container Linux 节点[通过 Rancher 启动 Kubernetes](launch-kubernetes-with-rancher.md) 时，需要在 [Cluster Config 文件](configuration/rke1.md#rke-集群配置文件参考)中使用如下配置：
...
使用[主机驱动](../rancher-admin/global-configuration/provisioning-drivers/provisioning-drivers.md#主机驱动)时会自动启用 Docker 服务。

### Windows 节点

运行 Windows Server 节点必须使用 Docker 企业版。

Windows 节点只能用于 Worker 节点。请参阅[配置 Windows 自定义集群](custom-clusters/windows/use-windows-clusters.md)。
...
需要开放的端口根据下游集群的启动方式而有所不同。以下列出了需要为不同[集群创建选项](cluster-deployment.md)打开的端口。
...
在[下游集群端口要求](../installation-and-upgrade/requirements/port-requirements.md#下游-kubernetes-集群节点)中，你可以找到在各种情况下使用的端口的详细信息。
...
有关强化指南的更多信息，以及了解哪个指南版本对应于你的 Rancher 和 Kubernetes 版本，请参阅[安全](../security/security-overview.md#rancher-加固指南)。

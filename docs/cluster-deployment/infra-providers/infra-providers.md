...
After cloud credentials are created, the user can start [managing the cloud credentials that they created](../../rancher-admin/users/settings/manage-cloud-credentials.md).

### Node Drivers

If you don't find the node driver that you want to use, you can see if it is available in Rancher's built-in [node drivers and activate it](../../rancher-admin/global-configuration/provisioning-drivers/manage-node-drivers.md#activatingdeactivating-node-drivers), or you can [add your own custom node driver](../../rancher-admin/global-configuration/provisioning-drivers/manage-node-drivers.md#adding-custom-node-drivers).
...
For RKE2 cluster templates, please refer to [this page](../../cluster-admin/manage-clusters/cluster-templates.md#rke2-cluster-template) for additional information.
...
In our [recommended cluster architecture](../production-checklist/recommended-cluster-architecture.md), we outline how many nodes of each role clusters should have:

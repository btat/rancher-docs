...
在创建 [Amazon EC2 集群](../infra-providers/aws/aws.md)时，你必须在创建**节点模板**时填写创建的 IAM 角色的 **IAM Instance Profile Name**（不是 ARN）。

创建[自定义集群](../custom-clusters/custom-clusters.md)时，你必须手动将 IAM 角色附加到实例。
...
创建 [Amazon EC2 集群](../infra-providers/aws/aws.md)时，会自动为创建的节点配置 `ClusterID`。其他资源仍然需要手动标记。

...
For more information on RKE node roles, see the [best practices.](production-checklist/production-checklist.md#cluster-architecture)
...
5. If you are importing a generic Kubernetes cluster in Rancher, perform the following steps for setup:<br/>
  a. Click **Agent Environment Variables** under **Cluster Options** to set environment variables for [rancher cluster agent](about-rancher-agents.md). The environment variables can be set using key value pairs. If rancher agent requires use of proxy to communicate with Rancher server, `HTTP_PROXY`, `HTTPS_PROXY` and `NO_PROXY` environment variables can be set using agent environment variables.<br/>
...
- [Manage cluster access](../rancher-admin/users/authn-and-authz/manage-role-based-access-control-rbac/cluster-and-project-roles.md) through role-based access control
- Enable [monitoring, alerts and notifiers](../observability/monitoring-and-dashboards/monitoring-and-alerting.md)
- Enable [logging](../observability/logging/logging.md)
- Enable [Istio](../observability/istio/istio.md)
...
- The ability to [upgrade the Kubernetes version](../cluster-admin/backups-and-restore/backups-and-restore.md)
  :::danger
...
See [Cluster Management Capabilities by Cluster Type](cluster-deployment.md) for more information about what features are available for managing registered clusters.
...
- These steps will alter the configuration of the downstream RKE2 and K3s clusters and deploy the `kube-api-authn-webhook`. If a future implementation of the ACE requires an update to the `kube-api-authn-webhook`, then this would also have to be done manually. For more information on this webhook, click [here](../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md#about-the-kube-api-auth-authentication-webhook).

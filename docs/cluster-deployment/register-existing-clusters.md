...
有关 RKE 节点角色的更多信息，请参阅[最佳实践](production-checklist/production-checklist.md#集群架构)。
...
5. 如果你在 Rancher 中导入一个通用 Kubernetes 集群，请执行以下步骤：<br/>
   a. 点击**集群选项**下的 **Agent 环境变量**，为 [rancher cluster agent](about-rancher-agents.md) 设置环境变量。你可以使用键值对设置环境变量。如果 Rancher Agent 需要使用代理与 Rancher Server 通信，则可以使用 Agent 环境变量设置 `HTTP_PROXY`，`HTTPS_PROXY` 和 `NO_PROXY` 环境变量。<br/>
...
- 通过 RBAC [管理集群访问](../rancher-admin/users/authn-and-authz/manage-role-based-access-control-rbac/cluster-and-project-roles.md)
- 启用[Monitoring、告警和 Notifiers](../observability/monitoring-and-dashboards/monitoring-and-alerting.md)
- 启用 [Logging](../observability/logging/logging.md)
- 启用 [Istio](../observability/istio/istio.md)
- 管理项目和工作负载
...
- [升级 Kubernetes 版本](../cluster-admin/backups-and-restore/backups-and-restore.md)的能力
  :::danger
...
有关可用于管理已注册集群的功能，请参阅[按集群类型划分的集群管理功能](cluster-deployment.md)。
...
授权集群端点 (ACE) 已支持注册的 RKE2 和 K3s 集群。此支持还包括你在下游集群上启用 ACE 的手动步骤。有关授权集群端点的更多信息，请单击[这里](../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md)。
...
- 这些步骤将改变下游 RKE2 和 K3s 集群的配置并部署 `kube-api-authn-webhook`。如果 ACE 的未来实现需要更新 `kube-api-authn-webhook`，那么这也必须手动完成。有关此 webhook 的更多信息，请单击[此处](../cluster-admin/manage-clusters/access-clusters/authorized-cluster-endpoint.md#关于-kube-api-auth-身份验证-webhook)。

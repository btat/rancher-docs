...
See [Troubleshooting: Logging](../observability/logging/troubleshooting.md)
...
When the IP address of the node changed, Rancher lost connection to the node, so it will be unable to clean the node properly. See [Cleaning cluster nodes](../cluster-admin/manage-clusters/clean-cluster-nodes.md) to clean the node.
...
You can add more arguments/binds/environment variables via the [Config File](../cluster-deployment/configuration/rke1.md#rke-cluster-config-file-reference) option in Cluster Options. For more information, see the [Extra Args, Extra Binds, and Extra Environment Variables](https://rancher.com/docs/rke/latest/en/config-options/services/services-extras/) in the RKE documentation or browse the [Example Cluster.ymls](https://rancher.com/docs/rke/latest/en/example-yamls/).

...
> **注意**：有关配置以下选项的更多信息，请参阅[备份配置参考页面](backup.md)。
...
这个例子表明，如果运行 `rancher-backup` 的节点拥有这些[访问 S3 的权限](backup.md#ec2-节点访问-s3-的-iam-权限)，就不必提供 AWS 的凭证密文来创建备份。
...
> **注意**：有关配置以下选项的更多信息，请参阅[恢复配置参考页面](restore.md)。
...
这个例子表明，如果运行 `rancher-backup` 的节点拥有这些[访问 S3 的权限](backup.md#ec2-节点访问-s3-的-iam-权限)，就不必提供 AWS 的凭证密文来从备份中还原。

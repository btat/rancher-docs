...
You must first [install](../back-up.md) the `rancher-backup` operator. After you do so, you can access the **Backup: Create** page:
...
For example, when [migrating Rancher to a new cluster](../migrate-to-a-new-cluster.md) the file is used to re-create the secret in the new cluster.
...
1. **Credential Secret** (optional): If you need an AWS access key or secret key to access an S3 bucket, [create a secret](../../../security/secrets-hub.md) using your credentials, with keys and directives named `accessKey` and `secretKey`. The secret can be in any namespace. An example secret is [here](#example-credentialsecret). This directive is unnecessary if the nodes running your operator are in EC2 and assigned [IAM permissions to access S3](#iam-permissions-for-ec2-nodes-to-access-s3). The **Credential Secret** dropdown lists secrets in all namespaces.
...
| ---------------- | ---------------- | ------------ |
| `credentialSecretName` |  If you need an AWS access key or secret key to access an S3 bucket, [create a secret](../../../security/secrets-hub.md) using your credentials, with keys and directives named `accessKey` and `secretKey`. The secret can be in any namespace. An example secret is [here](#example-credentialsecret). This directive is unnecessary if the nodes running your operator are in EC2 and assigned [IAM permissions to access S3](#iam-permissions-for-ec2-nodes-to-access-s3). |  |

...
* Follow the instructions from this page for restoring rancher on the same cluster where it was backed up from. In order to migrate rancher to a new cluster, follow the steps to [migrate rancher.](migrate-to-a-new-cluster.md)
...
* If you need to restore Rancher to a previous version after an upgrade, see the [rollback documentation.](../../installation-and-upgrade/rollbacks.md)
...
1. Follow these [instructions](migrate-to-a-new-cluster.md) to install the rancher-backup Helm chart on the existing cluster and restore the previous state.
    1. Omit Step 3.
...
1. Click **Create**.
1. Create the Restore with the form, or with YAML.  For creating the Restore resource using form, refer to the [configuration reference](configuration/restore.md) and to the [examples.](configuration/examples.md)
1. For using the YAML editor, we can click **Create > Create from YAML**. Enter the Restore YAML.
    ...
    ```
      ...
      For help configuring the Restore, refer to the [configuration reference](configuration/restore.md) and to the [examples.](configuration/examples.md)

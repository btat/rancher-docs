...
These instructions assume that you have [created a backup](back-up.md) and  already installed a new Kubernetes cluster where Rancher will be deployed. The backup is specific to the Rancher application and can only migrate the Rancher application.
...
1. Create a `Restore` object:
   ...
   1. Create an [encryption configuration file](configuration/backup.md#encryption)
...
Follow the steps to [install cert-manager](../../installation-and-upgrade/install-rancher.md#4-install-cert-manager) in the documentation about installing cert-manager on Kubernetes.

...
1. 点击 **Rancher 备份**。
1. 单击**安装**。
1. 可选：配置默认存储位置。如需获取帮助，请参见[配置](configuration/storage.md)。
1. 单击**安装**。
...
备份是通过创建 Backup 自定义资源实现的。如需查看教程，请参见[本页面](back-up.md)。

## 还原 Rancher

还原是通过创建 Restore 自定义资源实现的。如需查看教程，请参见[本页面](restore.md)。

## 将 Rancher 迁移到新集群

你可以按照[这些步骤](migrate-to-a-new-cluster.md)执行迁移。
...
如需了解各个选项的配置，请参见[本页面](configuration/storage.md)。

### 示例 YAML 文件：Rancher Backup Helm Chart

当使用 Helm CLI 安装时，可以使用示例 [values.yaml 文件](configuration/storage.md#rancher-backup-helm-chart-的示例-valuesyaml) 来配置 `rancher-backup-operator`。

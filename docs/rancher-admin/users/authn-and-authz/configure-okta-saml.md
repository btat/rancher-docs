...
[Configure the settings](openldap/reference.md) for the OpenLDAP server, groups and users. Note that nested group membership isn't available.

> Before you proceed with the configuration, please familiarise yourself with [external authentication configuration and principal users](authn-and-authz.md#external-authentication-configuration-and-principal-users).

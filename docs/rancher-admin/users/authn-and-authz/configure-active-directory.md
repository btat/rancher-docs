...
Rancher uses LDAP to communicate with the Active Directory server. The authentication flow for Active Directory is therefore the same as for the [OpenLDAP authentication](openldap/openldap.md) integration.

:::note

Before you start, please familiarise yourself with the concepts of [External Authentication Configuration and Principal Users](authn-and-authz.md#external-authentication-configuration-and-principal-users).

...
When a user from an [external authentication source](../authn-and-authz.md) signs into Rancher for the first time, they're automatically assigned a set of global permissions (hereafter, permissions). By default, after a user logs in for the first time, they are created as a user and assigned the default `user` permission. The standard `user` permission allows users to login and create clusters.
...
* You have set up an [external authentication provider](../authn-and-authz.md#external-vs-local-authentication)
* The external authentication provider supports [user groups](../manage-users-and-groups.md)

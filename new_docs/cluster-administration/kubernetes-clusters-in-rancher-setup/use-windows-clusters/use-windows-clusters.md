...
When provisioning a [custom cluster](../../../installation-and-upgrade/rancher-server-configuration/use-existing-nodes/use-existing-nodes.md) using Rancher, Rancher uses RKE (the Rancher Kubernetes Engine) to install Kubernetes on your existing nodes.
...
The general node requirements for networking, operating systems, and Docker are the same as the node requirements for a [Rancher installation](../../../installation-and-upgrade/requirements/installation-requirements.md).
...
Before provisioning a new cluster, be sure that you have already installed Rancher on a device that accepts inbound network traffic. This is required in order for the cluster nodes to communicate with Rancher. If you have not already installed Rancher, please refer to the [installation documentation](../../../installation-and-upgrade/installation-and-upgrade.md) before proceeding with this guide.
...
The instructions for creating a Windows cluster on existing nodes are very similar to the general [instructions for creating a custom cluster](../../../installation-and-upgrade/rancher-server-configuration/use-existing-nodes/use-existing-nodes.md) with some Windows-specific requirements.
...
1. Optional: If you click **Show advanced options,** you can customize the settings for the [Rancher agent](../../../installation-and-upgrade/rancher-server-configuration/use-existing-nodes/rancher-agent-options.md) and [node labels.](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/)
...
- **Access your cluster with the kubectl CLI, using the authorized cluster endpoint:** Follow [these steps](../../../../docs/how-to-guides/new-user-guides/manage-clusters/access-clusters/use-kubectl-and-kubeconfig.md#authenticating-directly-with-a-downstream-cluster) to access your cluster with kubectl directly, without authenticating through the Rancher server. We recommend setting up this alternative method to access your cluster so that in case you can’t connect to Rancher, you can still access the cluster.

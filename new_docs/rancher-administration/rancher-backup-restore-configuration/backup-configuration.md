...
You must first [install](../../../docs/how-to-guides/new-user-guides/backup-restore-and-disaster-recovery/back-up-rancher.md) the `rancher-backup` operator. After you do so, you can access the **Backup: Create** page:
...
For example, when [migrating Rancher to a new cluster](../../../docs/how-to-guides/new-user-guides/backup-restore-and-disaster-recovery/migrate-rancher-to-new-cluster.md) the file is used to re-create the secret in the new cluster.
...
| ---------------- | ---------------- | ------------ |
| `credentialSecretName` |  If you need an AWS access key or secret key to access an S3 bucket, [create a secret](../../../docs/how-to-guides/new-user-guides/kubernetes-resources-setup/secrets.md) using your credentials, with keys and directives named `accessKey` and `secretKey`. The secret can be in any namespace. An example secret is [here](#example-credentialsecret). This directive is unnecessary if the nodes running your operator are in EC2 and assigned [IAM permissions to access S3](#iam-permissions-for-ec2-nodes-to-access-s3). |  |
...
For example Backup custom resources, refer to [this page.](../../../docs/reference-guides/backup-restore-configuration/examples.md#backup)

...
These instructions assume that you have [created a backup](../../../docs/how-to-guides/new-user-guides/backup-restore-and-disaster-recovery/back-up-rancher.md) and  already installed a new Kubernetes cluster where Rancher will be deployed. The backup is specific to the Rancher application and can only migrate the Rancher application.
...
1. Create a `Restore` object:
   ...
   1. Create an [encryption configuration file](../rancher-backup-restore-configuration/backup-configuration.md#encryption)
...
Follow the steps to [install cert-manager](../../installation-and-upgrade/installation/install-upgrade-on-a-kubernetes-cluster.md#4-install-cert-manager) in the documentation about installing cert-manager on Kubernetes.

...
See the [Backup Configuration documentation](../rancher-backup-restore-configuration/backup-configuration.md) for more information about the options, including encryption, to configure a backup.
...
There are two main restore scenarios, restoring to a cluster with Rancher running and restoring to a fresh cluster. You can only restore to a cluster with Rancher running if it's the same cluster the backup was taken from and the [`prune` option](../rancher-backup-restore-configuration/restore-configuration.md#prune-during-restore) is enabled during the restore. A restore has similar inputs as a backup. It requires a backup filename, the encryptionConfigSecret name, and the storage location.
...
See the [Restore Configuration documentation](../rancher-backup-restore-configuration/restore-configuration.md) for more information about the options to configure a restore.
